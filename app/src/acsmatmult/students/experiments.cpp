...
void runMatrixExperiment(unsigned int min, unsigned int max, unsigned int repeats, const std::string &file_out) {
  std::cout << " Implementing Baseline matrix multiplication." << std::endl;
  std::cout << "Matrix Multlication benchmark." << std::endl;
  // Attempt to open the file for writing
  std::ofstream fos(file_out);
  if (!fos.good()) {
    std::cerr << "Could not open file " + file_out << std::endl;
  ...
  std::cout << " Baseline matrix multiplication completed." << std::endl;
}

void runMatrixExperimentSIMD(unsigned int min, unsigned int max, unsigned int repeats, const std::string &file_out) {
  
  std::cout << "SIMD matrix multiplication experiment implemented." << std::endl;
  std::ofstream fos(file_out);
  if (!fos.good()) {
    std::cerr << "Could not open file " + file_out << std::endl;
    std::exit(-1);
  ...
  for (unsigned int e = min; e < max; e++) {
    ...
    for (unsigned int r = 0; r < repeats; r++) {
      t.start();  // Start the timer.
      // Multiply the matrices
      auto mat_resultf = multiplyMatricesSIMD(mat_a, mat_b);
      t.stop();  // Stop the timer.
      t.report(ss);
    ...
    for (unsigned int r = 0; r < repeats; r++) {
      t.start();
      auto mat_resultd = multiplyMatricesSIMD(mat_c, mat_d);
      t.stop();
...
void runMatrixExperimentOMP(unsigned int min,
  ...
  for (unsigned int e = min; e < max; e++) {
    ...
    for (unsigned int r = 0; r < repeats; r++) {
      ...
      // Multiply the matrices
      //auto mat_result = Matrix<float>::multiply(mat_a, mat_b);
      auto mat_resultf = multiplyMatricesOMP(mat_a, mat_b, threads);
      t.stop();  // Stop the timer.
      t.report(ss);
    ...
    for (unsigned int r = 0; r < repeats; r++) {
      t.start();
      auto mat_resultd = multiplyMatricesOMP(mat_c, mat_d, threads);
      t.stop();
  ...
  std::cout << " OpenMP matrix multiplication completed." << std::endl;
